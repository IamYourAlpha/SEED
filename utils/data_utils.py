...
def prepare_dataset_for_router(dataset_, train_batch, test_batch):
    ...
    if (dataset_ == 'fmnist'):
        ...
        transform_train = transforms.Compose([
            ...
            transforms.RandomErasing(probability = 0.5, sh = 0.3, r1 = 0.3)
            ])
    ...
    if (dataset_ == 'svhn'):
        ...
        testloader_single = data.DataLoader(testset, batch_size=1, shuffle=True, num_workers=0)
...
def prepeare_dataset_for_experts(dataset_, matrix, values, train_batch, test_batch, weighted_sampler=True):
    ...
    transform_train = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
        transforms.RandomErasing(probability = 0.5, sh = 0.1, r1 = 0.3)
    ])
    transform_test = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
