...
def train(epoch, model, experts, teacher, Q_loi, train_loader, train_loader_all_data, optimizer, teacher_temp=3, student_temp=5, stocastic_loss=False): 
    model.train()
...
def inference_with_experts_and_routers(test_loader, experts, router, original_=True, topk=2):
    ...
    for dta, target in test_loader:
        ...
        experts_output.append(output_raw*2) # append the router raw logits
...
def main():
    ...
    if (args.evaluate_only_router):
        test(router, test_loader_router, best_so_far=None, name='_', print_=True, save_wts=False)
    ...
    if (args.train_mode):
        ...
        while(True):
            ...
            Q_temp = ['3_5', '0_8', '1_9', '2_6', '2_3']
            #Q_temp = ['3_4_5', '2_3_5', '3_5_7', '4_5_7', '0_1_9', '2_4_6', '3_5_6', '2_3_4', '2_3_6', '1_8_9']
            
            if (args.topk == 2):
                 for elem in Q_temp:
                     if (current_expert in elem):
                         if (current_expert == elem[0]):
                             Q_common.add(elem[2])
                         if (current_expert == elem[2]):
                             Q_common.add(elem[0])
            if (args.topk == 3):
                for elem in Q_temp:
                    if (current_expert in elem):
                        ...
                        if (current_expert == elem[4]):
                            Q_common.add(elem[0])
                            Q_common.add(elem[2])
                                            
            Q_common = list(Q_common)
            print ("Q_common: {}".format(Q_common)) 
            #print ("Q_LOI: {}".format(Q_loi))
            print ("Current expert: {}".format(current_expert))
            ...
            for epoch in range(0, args.expert_epochs):
                ...
                experts[current_expert], eoptmizers[current_expert] = train(epoch,
                                                                            ...
                                                                            teacher_temp=3, # temp for experts
                                                                            student_temp=3,
                                                                            stocastic_loss=False # Do you want MS-NET LOSS
                                                                            )
    ...
    topk = args.topk
    original_ = False
    accuracy_exp, m, corrected_samples, expert_count = inference_with_experts_and_routers(test_loader_single, experts, router, original_, topk)
    #heatmap(m, ls, ls)
